...
# Smaller font for position labels
...
# Option to toggle position labels
show_labels = True  # Set to True by default to show position labels
...
def draw_info():
    ...
    labels_button_rect = pygame.Rect(info_x, board_offset_y + 120, 140, 30)
    pygame.draw.rect(screen, GREEN if show_labels else (150, 150, 150), labels_button_rect)
    label_text = "Hide Labels" if show_labels else "Show Labels"
    label_render = font.render(label_text, True, BLACK)
    screen.blit(label_render, (info_x + 10, board_offset_y + 125))
    
    if game_over:
        over_text = "Game Over!"
        over_render = font.render(over_text, True, BLACK)
        screen.blit(over_render, (info_x, board_offset_y + 160))
        # Draw clickable Restart button
        restart_button_rect = pygame.Rect(info_x, board_offset_y + 200, 100, 40)
        pygame.draw.rect(screen, GREEN, restart_button_rect)
        restart_text = font.render("Restart", True, BLACK)
        screen.blit(restart_text, (info_x + 10, board_offset_y + 210))
        # Draw clickable Exit button
        exit_button_rect = pygame.Rect(info_x, board_offset_y + 250, 100, 40)
        pygame.draw.rect(screen, RED, exit_button_rect)
        exit_text = font.render("Exit", True, BLACK)
        screen.blit(exit_text, (info_x + 25, board_offset_y + 260))
...
def draw_board():
    ...
    for pos, coord in positions.items():
        ...
        if show_labels:
            # Create a small white background for better visibility of text
            label_surface = label_font.render(pos, True, BLACK)
            label_rect = label_surface.get_rect(center=(pos_coord[0], pos_coord[1] - 20))
            # Add a white background behind text for better visibility
            bg_rect = label_rect.copy()
            bg_rect.inflate_ip(6, 4)  # Make background slightly larger than text
...
while running:
    for event in pygame.event.get():
        if event.type == pygame.QUIT:
            ...
            # Check for toggle labels button click
            labels_button_rect = pygame.Rect(info_x, board_offset_y + 120, 140, 30)
            if labels_button_rect.collidepoint(x, y):
                show_labels = not show_labels
                draw_board()
                continue
